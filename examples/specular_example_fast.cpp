...
int main(void)
{
	...
	auto g = [&t,&r,a,b](Eigen::Vector<T, 4> const& x, Eigen::Vector<T,4> & grad){return specular_gradient(x,t,r,a,b,grad);};
	auto h = [&t,&r,a,b](Eigen::Vector<T, 4> const& x, Eigen::Matrix<T,4,4> & hess){return specular_hessian(x,t,r,a,b,hess);};

	Eigen::Vector<T,4> x0 = get_x0(t,r,a,b);

	Result<T,4> result;
	for (int i = 0; i < 50*86400; i++)
		result = newtons_method_fast(f,g,h,x0);
